...
AUTHORIZED_USERS = [1111065199521320961]
...
async def help(ctx):
    ...
    response = "<a:crownb:1118526977368936539> **KAKAPO BOT** <a:crownb:1118526977368936539> \n\n"
    for command in sorted_commands:
        response += f"<a:thunder:1118526480318734367> `{command.name}`,"
...
async def userinfo(ctx, member: discord.Member = None):
    ...
    user_info = [
        f"** <:user:1118526395769954334>• Username: {member.name}**",
        f"** <a:bluecheck:1119912784113717339> • Discriminator: {member.discriminator}**",
        f"** <a:bluecheck:1119912784113717339> • ID: {member.id}**",
        f"** <a:bluecheck:1119912784113717339> • Nickname: {member.nick}**",
        f"** <a:ARPGLIGHTNINGStrike:1119912607705477240> • Joined Discord: <t:{int(member.created_at.timestamp())}:d>**",
        f"** <a:ARPGLIGHTNINGStrike:1119912607705477240> • Joined Server: <t:{int(member.joined_at.timestamp())}:d>**"
    ]
...
async def selfbot(ctx):
    version = "BUZZ PREMIUM"
    language = "Python"
    author = "@Buzz.#0001 "
    total_commands = len(bot.commands)
    discord_link = "https://discord.gg/meFSqfzrYf"
    ...
    message = f"<a:crownb:1118526977368936539> **__BUZZ S3LFB0T__**\n\n" \
              f"**• Version: {version}\n" \
              f"• Lang: {language}\n" \
              f"• Created By: {author}\n" \
...
async def ltcprice(ctx):
    ...
    if response.status_code == 200:
        ...
        await ctx.send(f" <:litecoin:1119670483617579102> **The current price of Litecoin (LTC) is** ${price:.2f}")
    else:
...
async def getbal(ctx, ltcaddress):
    ...
    message = f"**__LTC ADRESS:__** `{ltcaddress}`\n"
    message += f"**<:white_money:1119672026806898740> LIVE FUNDS:** **${usd_balance:.2f} USD**\n"
    message += f"**<a:moneytoburn:1119672869476114464> TOTAL LTC FUNDED:** **${usd_total_balance:.2f} USD**\n"
    message += f"**<a:SA_zoomzoom:1119673182425714779> LOCKED LTC:** **${usd_unconfirmed_balance:.2f} USD**"
